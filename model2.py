...
class build_network():
    def __init__(self, learning_rate, size):
        img_size = 512
        self.size = mask_size = size
        self.x = tf.placeholder(tf.float32, shape=[None, mask_size, mask_size, 3], name='x')
        self.label = tf.placeholder(tf.int64, shape=[None], name="label")

        self.img = tf.placeholder(tf.float32, shape=[None, img_size, img_size, 3], name='img')
    ...
    def combine(self, img, mask):
            x = tf.layers.conv2d(img, filters=64, kernel_size=(3,3), strides=(2, 2), padding='same', activation=leaky_relu)
            x = tf.layers.conv2d(x, filters=64, kernel_size=(3,3), strides=(2, 2), padding='same', activation=leaky_relu)

            x2 = tf.layers.conv2d(mask, filters=64, kernel_size=(3,3), strides=(2, 2), padding='same', activation=leaky_relu)
